...
{ 	
  ...
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB|RCC_AHB1Periph_GPIOD|RCC_AHB1Periph_GPIOE|RCC_AHB1Periph_GPIOG, ENABLE);//使能PD,PE,PF,PG时钟  
  RCC_AHB3PeriphClockCmd(RCC_AHB3Periph_FSMC,ENABLE);//使能FSMC时钟  
  
  
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0;//PB15 推挽输出,控制背光
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0;//PF12,FSMC_A10
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;//复用输出
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;//推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100MHz
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
  ...
  GPIO_PinAFConfig(GPIOG,GPIO_PinSource0,GPIO_AF_FSMC);//PF12,AF12
  GPIO_PinAFConfig(GPIOG,GPIO_PinSource12,GPIO_AF_FSMC);
  
  
  readWriteTiming.FSMC_AddressSetupTime = 0;	 //地址建立时间（ADDSET）为1个HCLK 1/168M=6ns*16=96ns	
  ...
  writeTiming.FSMC_AddressSetupTime =0;	      //地址建立时间（ADDSET）为个HCLK =54ns 
  writeTiming.FSMC_AddressHoldTime = 0x00;	 //地址保持时间（A		
  writeTiming.FSMC_DataSetupTime = 0;		 //数据保存时间为6ns*9个HCLK=54ns
...
if(lcddev.id==0x5510)
  {
    ...
    //Source hold time
    LCD_WriteReg(0xB500,0x6b);
    LCD_WriteReg(0xB600,0x0A);
    //Gate EQ control
